...
class SparkMetricsSuite extends FunSuite with Logging {
  ...
  test("Metrics are captured correctly") {
    ...
    addValue(myMetrics.metric1, 100, "host1", "stage1")
    addValue(myMetrics.metric1, 110, "host1", "stage2")
    addValue(myMetrics.metric1, 111, "host2", "stage2")
    addValue(myMetrics.metric1, 112, "host2", "stage1")
    addValue(myMetrics.metric1, 113, "host3", "stage3")

    assert(fromNanos(myMetrics.metric1.overallTimings.getTotalTime) === 546)

    val stageTimings = myMetrics.metric1.timingsByStageName
    assert(fromNanos(stageTimings.get("stage1").get.getTotalTime) === 212)
    assert(fromNanos(stageTimings.get("stage2").get.getTotalTime) === 221)
    assert(fromNanos(stageTimings.get("stage3").get.getTotalTime) === 113)
  ...
  test("Metrics are rendered correctly") {
    ...
    addValue(myMetrics.metric1, 111, "host2", "stage2")
    addValue(myMetrics.metric1, 112, "host2", "stage1")
    addValue(myMetrics.metric2, 200, "host1", "stage1")
    addValue(myMetrics.metric2, 210, "host1", "stage2")
    addValue(myMetrics.metric2, 211, "host2", "stage2")
    addValue(myMetrics.metric2, 212, "host2", "stage1")
  ...
  private def getExpectedValuesByStage: Array[Array[String]] = {
    Array(
      ...
      Array("Metric 2", "stage2", "421 ms", "2", "210.5 ms", "210 ms", "211 ms"),
      Array("Metric 2", "stage1", "412 ms", "2", "206 ms", "200 ms", "212 ms"),
      Array("Metric 1", "stage2", "221 ms", "2", "110.5 ms", "110 ms", "111 ms"),
      Array("Metric 1", "stage1", "212 ms", "2", "106 ms", "100 ms", "112 ms"))
  }
  ...
  private def addValue(timer: TaskTimer, value: Long, host: String, stage: String) {
