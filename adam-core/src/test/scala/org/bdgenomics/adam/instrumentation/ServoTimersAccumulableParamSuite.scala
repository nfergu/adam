...
class ServoTimersAccumulableParamSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("Values accumulated correctly") {
    ...
    val root = new TimingPath(0, None)
    val child1 = new TimingPath(1, Some(root))
    val child2 = new TimingPath(2, Some(root))
    // This has the same ID as child2, but has (some) different ancestors
    val grandchild1 = new TimingPath(2, Some(child1))

    // First, record a single timing for each timer
    accumulable += new RecordedTiming(100000, "Timer 1", root)
    accumulable += new RecordedTiming(200000, "Timer 2", child1)
    accumulable += new RecordedTiming(300000, "Timer 3", child2)
    accumulable += new RecordedTiming(400000, "Timer 2", grandchild1)
    ...
    accumulable += new RecordedTiming(50000, "Timer 1", root)
    accumulable += new RecordedTiming(50000, "Timer 3", child2)
  ...
  test("Paths with different properties are treated as distinct timers") {
    ...
    val child1 = new TimingPath(2, Some(root), sequenceId = 101)
    val child2 = new TimingPath(2, Some(root), sequenceId = 102)
    val child3 = new TimingPath(2, Some(root), sequenceId = 101, isRDDOperation = true)

    // We should get four distinct timers here
    accumulable += new RecordedTiming(100000, "Timer 1", root)
    accumulable += new RecordedTiming(200000, "Timer 2", child1)
    accumulable += new RecordedTiming(300000, "Timer 3", child2)
    accumulable += new RecordedTiming(400000, "Timer 4", child3)
  ...
  test("Accumulable params can be merged together") {
    ...
    accumulable1 += new RecordedTiming(100000, "Timer 1", root)
    accumulable1 += new RecordedTiming(200000, "Timer 2", child1)
    accumulable1 += new RecordedTiming(300000, "Timer 3", child2)
    ...
    accumulable2 += new RecordedTiming(400000, "Timer 2", grandchild1)
    accumulable2 += new RecordedTiming(50000, "Timer 1", root)
    accumulable2 += new RecordedTiming(50000, "Timer 3", child2)
